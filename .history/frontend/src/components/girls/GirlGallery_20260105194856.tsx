...
import { getFullImageUrl, DEFAULT_FALLBACK_IMAGE } from '@/lib/utils/image';
...
export default function GirlGallery({ id, images, name }: GirlGalleryProps) {
  ...
  const mainImage = validImages[safeSelectedIndex] || validImages[0] || DEFAULT_FALLBACK_IMAGE;

  // Debug: Log images data only in development and only once on mount
  useEffect(() => {
    if (process.env.NODE_ENV === 'development' && validImages.length === 0) {
      console.warn('[GirlGallery] No valid images for:', name);
    }
  }, [name, validImages.length]);

  const handleImageError = (index: number, imageUrl?: string | null) => {
    // Only log in development to avoid console noise in production
    if (process.env.NODE_ENV === 'development') {
      console.warn('[GirlGallery] Image failed to load at index:', index, 'URL:', imageUrl || 'unknown');
    }
    setImageErrors(prev => new Set(prev).add(index));
  };

  const getImageUrl = (image: string | undefined | null, index: number) => {
    // Return fallback if image has error
    if (imageErrors.has(index)) {
      return DEFAULT_FALLBACK_IMAGE;
    }
    
    // Return fallback if image is invalid
    if (!image || typeof image !== 'string' || image.trim().length === 0) {
      return DEFAULT_FALLBACK_IMAGE;
    }
    
    // Process and return the URL (getFullImageUrl already handles fallback)
